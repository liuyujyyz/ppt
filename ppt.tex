...
\frametitle{Problem Definition}
	...
	\item Output: a feasible set, which optimizes the expected value of the best choice.
	\end{itemize}
    \onslide<2->{
\begin{problem}[$\MM$]
Input:
...
\begin{frame}
    ...
    Find a $T$ such that:
    \begin{itemize}
        \item For all set $S\in \calF$, $\Exp[\sum_{i\in S} \max(X_i - T, 0)] \leq T$ (provide a upper bound for optimal solution);
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Define Signature for $\bar X_i$ using $q_{i,j} = \Pr[Y_{i,j} = 0]$:
...
\begin{frame}
    ...
    \begin{enumerate}
        \onslide<1->{\item Enumerate possible signatures of sets.}
        \onslide<2->{\item For each signature, try to find a candidate with such signature.\\}
            \only<3-3>{ {\color{blue} Coordinates of signatures are integer multiple of $\varepsilon^4/n$.\\ 
            Encode one signature to one integer.\\
    Reduce finding a corresponding solution to \ES.\\}}
\onslide<4->{        \item Return a candidate with the best objective value as solution.}
    \end{enumerate}
\end{frame}
